...
namespace Lang.Parsing
{
  class SourceFile
  {
    ...
    SourceFile(string path, string text) => (Path, Text) = (path, text);
    public static SourceFile FromString(string src) => new SourceFile("<mem>", src);
    public static SourceFile Read(string path) => new SourceFile(path, File.ReadAllText(path));
